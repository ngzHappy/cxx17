...
#ifndef BOTAN_DLL
#define BOTAN_DLL _CPLUSPLUS_BASIC_LIBRARYSHARED_EXPORT
#endif

#ifndef Q_DECL_NOEXCEPT_EXPR
#define Q_DECL_NOEXCEPT_EXPR(...)
#endif
...
public:
    ...
    /**
      ...
      * otherwise.
      */
    bool operator!=(const MemoryRegion<T>& other) const { return (!(*this==other)); }
...
protected:
    ...
    /**
      ...
      */
    void init(bool locking,size_t length=0) { alloc=Allocator::get(locking); resize(length); }
...
private:
    ...
    void deallocate(T* p,size_t n) { if (alloc && p && n) alloc->deallocate(p,sizeof(T)*n); }
...
public:
    ...
    /**
      ...
      */
    MemoryRegion<byte>& get_io_buffer(size_t size) { io_buffer.resize(size); return io_buffer; }

    /**
      * @return number of bits collected so far
      */
    size_t bits_collected() const { return static_cast<size_t>(collected_bits); }

    /**
      * @return if our polling goal has been achieved
      */
    bool polling_goal_achieved() const { return (collected_bits>=entropy_goal); }
...
public:
    using _Super::_Super;
    inline Exception();
    inline Exception(const containers::string &);
    inline Exception(containers::string &&);
...
public:
    ...
    inline Invalid_Argument(const containers::string &);
    inline Invalid_Argument(containers::string &&);
    inline const char *what()const noexcept(true) override;
};
...
private:
    ...
    RandomNumberGenerator& operator=(const RandomNumberGenerator&) { return (*this); }
};
...
public:
    ...
    /**
      ...
      */
    bool arg_count_between(size_t lower,size_t upper) const { return ((arg_count()>=lower)&&(arg_count()<=upper)); }
    ...
    /**
      ...
      */
    containers::string cipher_mode() const { return (mode_info.size()>=1)?mode_info[0]:""; }

    /**
  * @return cipher mode padding (if any)
  */
    containers::string cipher_mode_pad() const { return (mode_info.size()>=2)?mode_info[1]:""; }
...
public:
    ...
    /**
      ...
      * Must be of length block_size().
      */
    void encrypt(const byte in[],byte out[]) const { encrypt_n(in,out,1); }
    ...
    /**
      ...
      * Must be of length block_size().
      */
    void decrypt(const byte in[],byte out[]) const { decrypt_n(in,out,1); }
...
public:
    ...
    /**
      ...
      */
    void cipher1(byte buf[],size_t len) { cipher(buf,buf,len); }
...
public:
     ...
     */
    word word_at(size_t n) const { return ((n<size())?reg[n]:0); }
...
* @return true if all primality tests passed, otherwise false
*/
inline bool quick_check_prime(const BigInt& n,RandomNumberGenerator& rng) { return primality_test(n,rng,0); }
...
* @return true if all primality tests passed, otherwise false
*/
inline bool check_prime(const BigInt& n,RandomNumberGenerator& rng) { return primality_test(n,rng,1); }
...
* @return true if all primality tests passed, otherwise false
*/
inline bool verify_prime(const BigInt& n,RandomNumberGenerator& rng) { return primality_test(n,rng,2); }
...
public:
    ...
    /**
      ...
      */
    bool is_zero() const { return (coord_x.is_zero()&&coord_z.is_zero()); }
...
inline PointGFp OS2ECP(const MemoryRegion<byte>& data,const CurveGFp& curve) { return OS2ECP(&data[0],data.size(),curve); }
...
template<>
inline void swap<Botan::PointGFp>(Botan::PointGFp& x,Botan::PointGFp& y) { x.swap(y); }
...
public:
    ...
    /**
      ...
      */
    MemoryVector<byte> DER_domain() const { return domain().DER_encode(domain_format()); }

    /**
      * Get the domain parameter encoding to be used when encoding this key.
      * @result the encoding to use
      */
    EC_Group_Encoding domain_format() const { return domain_encoding; }
protected:
...
public:
    ...
    size_t message_part_size() const { return domain().get_order().bytes(); }
...
public:
    ...
    AlgorithmIdentifier pkcs8_algorithm_identifier() const { return EC_PublicKey::algorithm_identifier(); }
};
...
public:
    ...
    HAS_160(): MDx_HashFunction(64,false,true),X(20),digest(5) { clear(); }
private:
...
public:
    ...
    /**
      ...
      * @param options a string listing initialization options
      */
    LibraryInitializer(const containers::string& options="") { LibraryInitializer::initialize(options); }
...
private:
    ...
    Algorithm_Factory& operator=(const Algorithm_Factory&) { return (*this); }
...
public:
    ...
    bool valid_iv_length(size_t iv_len) const { return cipher->valid_iv_length(iv_len); }
    ...
    /**
      ...
      * @return true if the key length is valid, false otherwise
      */
    bool valid_keylength(size_t length) const { return cipher->valid_keylength(length); }
...
public:
    ...
    /**
      ...
      * @return true if the key length is valid, false otherwise
      */
    bool valid_keylength(size_t length) const { return mac->valid_keylength(length); }
...
public:
    ...
    void add_entropy_source(EntropySource* es) { rng->add_entropy_source(es); }

    void add_entropy(const byte in[],size_t len) { rng->add_entropy(in,len); }
...
public:
    ...
    SHA_384(): MDx_HashFunction(128,true,true,16),digest(8) { clear(); }
private:
    void compress_n(const byte[],size_t blocks);
    void copy_out(byte[]);
...
public:
    ...
    SHA_512(): MDx_HashFunction(128,true,true,16),digest(8) { clear(); }
private:
    void compress_n(const byte[],size_t blocks);
    void copy_out(byte[]);
...
public:
    ...
    MD2(): X(48),checksum(16),buffer(16) { clear(); }
private:
    void add_data(const byte[],size_t);
...
public:
    ...
    bool valid_iv_length(size_t iv_len) const { return (iv_len==8); }
...
protected:
    /**
      ...
      * @return const reference to the key schedule
      */
    const SecureVector<u32bit>& get_round_keys() const { return round_key; }
...
public:
    ...
    MD5(): MDx_HashFunction(64,false,true),M(16),digest(4) { clear(); }
protected:
    void compress_n(const byte[],size_t blocks);
    void copy_out(byte[]);
...
public:
    ...
    bool valid_keylength(size_t key_len) const { return cipher->valid_keylength(key_len); }
...
public:
    ...
    RIPEMD_160(): MDx_HashFunction(64,false,true),M(16),digest(5) { clear(); }
private:
    void compress_n(const byte[],size_t blocks);
    void copy_out(byte[]);
...
public:
    ...
    Whirlpool(): MDx_HashFunction(64,true,true,32),M(8),digest(8) { clear(); }
private:
    void compress_n(const byte[],size_t blocks);
    void copy_out(byte[]);
...
public:
    ...
    SHA_224(): MDx_HashFunction(64,true,true),digest(8) { clear(); }
private:
    void compress_n(const byte[],size_t blocks);
    void copy_out(byte[]);
...
public:
    ...
    SHA_256(): MDx_HashFunction(64,true,true),digest(8) { clear(); }
private:
    void compress_n(const byte[],size_t blocks);
    void copy_out(byte[]);
...
public:
    ...
    BMW_512(): MDx_HashFunction(128,false,true),H(16),M(16),Q(32) { clear(); }
private:
    void compress_n(const byte input[],size_t blocks);
...
public:
    ...
    bool valid_iv_length(size_t iv_len) const { return (iv_len==cipher->block_size()); }
...
public:
    ...
    /**
      ...
      */
    static bool has_rdtsc() { return x86_processor_flags_has(CPUID_RDTSC_BIT); }

    /**
      * Check if the processor supports SSE2
      */
    static bool has_sse2() { return x86_processor_flags_has(CPUID_SSE2_BIT); }

    /**
      * Check if the processor supports SSSE3
      */
    static bool has_ssse3() { return x86_processor_flags_has(CPUID_SSSE3_BIT); }

    /**
      * Check if the processor supports SSE4.1
      */
    static bool has_sse41() { return x86_processor_flags_has(CPUID_SSE41_BIT); }

    /**
      * Check if the processor supports SSE4.2
      */
    static bool has_sse42() { return x86_processor_flags_has(CPUID_SSE42_BIT); }

    /**
      * Check if the processor supports extended AVX vector instructions
      */
    static bool has_avx() { return x86_processor_flags_has(CPUID_AVX_BIT); }

    /**
      * Check if the processor supports AES-NI
      */
    static bool has_aes_ni() { return x86_processor_flags_has(CPUID_AESNI_BIT); }

    /**
      * Check if the processor supports PCMULUDQ
      */
    static bool has_pcmuludq() { return x86_processor_flags_has(CPUID_PCMUL_BIT); }

    /**
      * Check if the processor supports MOVBE
      */
    static bool has_movbe() { return x86_processor_flags_has(CPUID_MOVBE_BIT); }

    /**
      * Check if the processor supports RDRAND
      */
    static bool has_rdrand() { return x86_processor_flags_has(CPUID_RDRAND_BIT); }
...
public:
    ...
    /**
      ...
      */
    MemoryVector<byte> public_value() const { return EC2OSP(public_point(),PointGFp::UNCOMPRESSED); }
...
public:
    ...
    MemoryVector<byte> public_value() const { return ECDH_PublicKey::public_value(); }
};
...
public:
    ...
    bool valid_iv_length(size_t iv_len) const { return (iv_len==8||iv_len==24); }
...
public:
    ...
    RIPEMD_128(): MDx_HashFunction(64,false,true),M(16),digest(4) { clear(); }
private:
    void compress_n(const byte[],size_t blocks);
    void copy_out(byte[]);
...
public:
    ...
    MD4(): MDx_HashFunction(64,false,true),M(16),digest(4) { clear(); }
protected:
    void compress_n(const byte input[],size_t blocks);
    void copy_out(byte[]);
...
public:
    ...
    bool valid_iv_length(size_t iv_len) const { return (iv_len%4==0&&iv_len<=16); }
...
public:
    ...
    bool valid_iv_length(size_t iv_len) const { return (iv_len<=permutation->block_size()); }
    ...
    OFB* clone() const { return new OFB(permutation->clone()); }
...
public:
    ...
    CTR_BE* clone() const { return new CTR_BE(permutation->clone()); }
